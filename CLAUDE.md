...
- `pam/retrieval/` — `search.retrieve()` is the read entry point. Pipeline: `query_parser` → FTS + vector candidate selection (merged) → `graph_expander` (bounded by `EDGE_WEIGHT_EXPANSION_THRESHOLD`; traversal pattern is fixed, not BFS over a depth ceiling) → `ranker` → `RetrievalResult`. Query parsing has a deterministic fallback that handles ISO dates, `today`/`yesterday`/`this week`, simple relation phrasing — the LLM path is optional.
